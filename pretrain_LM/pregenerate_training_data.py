...
for epoch in trange(epochs_to_generate, desc='Epoch'):
    ...
    intances = []
    pos_cnt, neg_cnt = 0, 0
    for idx, d in tqdm(enumerate(json.load((Path(data_dir)/'train_data_me.json').open()))):
        seq_a = d['text']
    ...
    metrics_filename.write(json.dumps({'num_training_instance': len(intances)}))
