...
class ObjectModel(BertPreTrainedModel):
    ...
    def forward(self, x_ids, x_seg, x_mask):
        _, pooled_output = self.bert(x_ids, token_type_ids=x_seg, attention_mask=x_mask, output_all_encoded_layers=False)  # [b,h]

        o = torch.sigmoid(self.linear(pooled_output))
